...
public record PlaceOrderRequest(
        ...
        @Size (max = 128) String quoteId)
{

    public record OrderLineRequest(@NotBlank (message = "sku is required")
            @Size (min = 1, max = 64)
            @Pattern (regexp = "^[A-Za-z0-9._-]+$", message = "sku has an invalid format") String sku,
            @jakarta.validation.constraints.Min (value = 1, message = "quantity must be >= 1")
            @jakarta.validation.constraints.Max (value = 100, message = "quantity must be <= 100") int quantity) {
    }
}
